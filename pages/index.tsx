...
const Home: NextPage = () => {
  ...
  const updatePrices = async () => {
    ...
    if (secret?.CONSUMER_KEY) {
      axios.get("/api/sid?customer_key=" + secret.CONSUMER_KEY).then((res) => {
        ...
        setSID(latestSID);
        axios.put("/api/sid?customer_key=" + secret.CONSUMER_KEY, {
          SID: [
            ...SID,
            ...SIDtemp
          ]
  ...
  const retry = async () => {
    ...
    await setLoading(false)
    await setItems(items.filter(item => item.status === STATUS.RETRY))
    await setRetry(false)
  }

  // rounds a number down
  const roundNumber = (num: number | string): number => {
    return Number(Math.floor((Number(Number(num).toFixed(2)) + Number.EPSILON) * 100) / 100)
  }
  ...
  const fetchitems = () => {
    ...
    if (!secret || !secretsReady || !amountToUpdate) {
      setShowSecrets(true)
      return
    }
    ...
    axios.post("/api/bl", {
      ...
      parts = parts.filter(p => SID.indexOf(p.inventory_id) === -1)
      // if the filtered is empty, reset the parts with the backup
      if (parts.length === 0) {
      ...
      for await (const part of parts) {
        ...
        // check if enough items processed
        if (itemsAmountStart >= itemsToDo) {
          break;
        }

        // check if the item is already in the items
        let partAlreadyInItems: boolean = items.some((item) => item.id === part.inventory_id)
        if (partAlreadyInItems) {
          continue;
        }
        ...
        await axios.post("/api/bl", {
          ...
          // request failed add call and continue. maybe recheck secrets.
          currentCalls++
          setCalls(currentCalls)
        })
      }
  ...
  const getSVGbasedOnStatus = (status: STATUS, onClickCallBack?: Function) => {
    let svg = <></>
    let clickable = false
    let className = ""
    switch (status) {
      ...
      <div>
        <table className='bg-sky-50 w-full text-left '>
          ...
          <tbody>
            ...
            ${item.status === STATUS.SUCCESS && ' bg-green-500'} 
            ${item.status === STATUS.ERROR && ' bg-red-500'}
            ${(
                    item.status === STATUS.SKIP_EQUAL ||
                    item.status === STATUS.SKIP_ZERO ||
                    item.status === STATUS.SKIP_MANUAL
                  ) && ' bg-amber-200'}
            `} key={id}>
                    ...
                    {getSVGbasedOnStatus(item.status, () => {
                      ...
                      if (item.status === STATUS.NOT_PROCESSED) {
                        setItems(items.map(_item => {
                          if (_item.id === item.id) {
                            _item.status = STATUS.SKIP_MANUAL
                          }
                          return _item
                        }))
                      } else if (item.status === STATUS.SKIP_MANUAL) {
                        setItems(items.map(_item => {
                          if (_item.id === item.id) {
                            _item.status = STATUS.NOT_PROCESSED
                          }
                          return _item
                        }))
                      } else
                      ...
                      } else if (item.status === STATUS.RETRY) {
                        setItems(items.map(_item => {
                          if (_item.id === item.id) {
                            _item.status = STATUS.ERROR
                          }
